...
const EMPTY = { nom: '', prenom: '', email: '', entreprise: '' }

export default function Newsletter({ user }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) showToast('Cet email est déjà inscrit.', 'error')
  ...
  const confirmDelete = async () => {
    ...
    showToast('Abonné retiré'); load(); setDeleteTarget(null)
  }
  ...
  const exportCSV = () => {
    ...
    showToast(`${subs.length} abonnés exportés`)
  }

  return (
    <Layout user={user}>
      <div className="page-header">
        <div><div className="page-title">Newsletter</div><div className="page-sub">{subs.length} abonné(s) · Export CSV disponible</div></div>
        <button className="btn btn-primary" onClick={exportCSV}>Export CSV ↓</button>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        <div>
          <div className="section-title">Formulaire inscription — vue partenaire</div>
          {submitted ? (
            ...
            <div className="card card-pad">
              <p style={{ fontSize: 13, color: '#6b7280', marginBottom: 16 }}>Restez informé des dernières actualités Motul Africa.</p>
              <form onSubmit={handleSubmit}>
                <div className="form-grid">
                  ...
                  <div className="form-full"><label className="form-label">Email <span className="form-req">*</span></label><input required type="email" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} /></div>
                  <div className="form-full"><label className="form-label">Entreprise</label><input value={form.entreprise} onChange={e => setForm(p => ({ ...p, entreprise: e.target.value }))} placeholder="Optionnel" /></div>
                </div>
        ...
        <div>
          <div className="section-title">Abonnés — Back-office</div>
          ...
          <div className="table-card">
            <div className="table-head" style={{ gridTemplateColumns: '2fr 1fr 40px' }}>
              {['Email', 'Date', ''].map((h, i) => <span key={i} className="th">{h}</span>)}
            </div>
            {loading ? <div className="empty-state">Chargement...</div>
              : subs.length === 0 ? <div className="empty-state">Aucun abonné.</div>
              : subs.map(s => (
                <div key={s.id} className="table-row" style={{ gridTemplateColumns: '2fr 1fr 40px' }}>
                  <div>
                  ...
                  <button className="btn" style={{ fontSize: 10, padding: '2px 6px', color: '#dc2626', borderColor: '#fecaca' }} onClick={() => setDeleteTarget(s)}>✕</button>
                </div>
              ))
            }
          </div>
      ...
      <ConfirmModal open={!!deleteTarget} title="Retirer l'abonné" message={`Supprimer "${deleteTarget?.email}" de la newsletter ?`} confirmLabel="Retirer" danger onConfirm={confirmDelete} onCancel={() => setDeleteTarget(null)} />
      <Toast toast={toast} />
    </Layout>
  )
}
